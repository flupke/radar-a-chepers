...
async fn connect_and_run(
    ...
    let register_reply = channel
        .send(
            "register_device",
            register_device_payload(radar_device, test_mode),
        )
        .await
...
fn channel_send_error(event: &'static str, error: impl std::fmt::Display) -> eyre::Report {
...
fn known_device_registration_payload_error(payload: &Payload) -> Option<&'static str> {
    match payload {
        ...
        Payload::Binary(bytes) => known_device_registration_error(&String::from_utf8_lossy(bytes)),
    }
}
