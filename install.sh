...
validate_radar_device() {
  ...
  case "$RADAR_DEVICE" in
    rd03d)
      ;;
    ld2451)
      echo "error: LD2451 firmware is not implemented; hardware deployment is unavailable" >&2
      echo "       Use ./start.sh --radar-device ld2451 --fake-people --local-web for UI development." >&2
      exit 1
      ;;
    *)
